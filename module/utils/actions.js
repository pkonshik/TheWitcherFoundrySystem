...
import {addModifiersToFormula} from "../helpers/utils.js";

export async function ApplyDamage(actor, dmgType, location, totalDamage) {
    let armors = actor.getListByType(witcher.itemTypes.armor).filter(a => a.isEquipped());

    let headArmors = armors.filter(h => h.isOfLocation(witcher.locations.head) || h.isOfLocation(witcher.locations.fullCover))
    let torsoArmors = armors.filter(t => t.isOfLocation(witcher.locations.torso) || t.isOfLocation(witcher.locations.fullCover))
    let legArmors = armors.filter(l => l.isOfLocation(witcher.locations.leg) || l.isOfLocation(witcher.locations.fullCover))

    let naturalArmors = armors.filter(n => n.isOfSystemType(witcher.armorTypes.natural))

    let damageTypeLoc = actor.getDamageTypeByName(dmgType).label

    // We are performing attack to the specific enemy
    actor.attackLocations = witcher.locations.filter(l => l.isAttackLocation && !l.selectRandomEnemy)
    let locationOptions = ``
    actor.attackLocations.forEach(l => locationOptions += `<option value="${l.name}">${game.i18n.localize(l.alias)}</option>`);
    ...
    if (newLocation !== witcher.locations.random.name) {
        location = actor.getLocationObject(newLocation);
    }
    //todo modification to config
    if (addOilDmg) {
    ...
    if (actor.type === "character") {
        ...
        naturalArmors.forEach(armor => {
            ...
            switch (location.name) {
                case "Head":
                    totalSP = Number(totalSP) + Number(armor?.system.headStopping);
                    displaySP += `+${armor?.system.headStopping}`;
                    break;
                case "Torso":
                    totalSP = Number(totalSP) + Number(armor?.system.torsoStopping);
                    displaySP += `+${armor?.system.torsoStopping}`;
                    break;
                case "R. Arm":
                    totalSP = Number(totalSP) + Number(armor?.system.rightArmStopping);
                    displaySP += `+${armor?.system.rightArmStopping}`;
                    break;
                case "L. Arm":
                    totalSP = Number(totalSP) + Number(armor?.system.leftArmStopping);
                    displaySP += `+${armor?.system.leftArmStopping}`;
                    break;
                case "R. Leg":
                    totalSP = Number(totalSP) + Number(armor?.system.rightLegStopping);
                    displaySP += `+${armor?.system.rightLegStopping}`;
                    break;
                case "L. Leg":
                    totalSP = Number(totalSP) + Number(armor?.system.leftLegStopping);
                    displaySP += `+${armor?.system.leftLegStopping}`;
                    break;
            }
    ...
    if (actor.type === "character") {
        switch (location.name) {
            case witcher.locations.head.name:
...
function getArmors(armors) {
    ...
    armors.forEach(item => {
        if (item.isOfSystemType(witcher.armorTypes.light)) {
            lightCount++;
            lightArmor = item
        }
        if (item.isOfSystemType(witcher.armorTypes.medium)) {
            mediumCount++;
            mediumArmor = item
        }
        if (item.isOfSystemType(witcher.armorTypes.heavy)) {
            heavyCount++;
            heavyArmor = item
        }
    });
...
export function BlockAttack(actor) {
    let weapons = actor.items.filter(function (item) {
        return item.type === "weapon" && !item.system.isAmmo && witcher.meleeSkills.includes(item.system.attackSkill)
    });
...
export function ExecuteDefence(actor, attackType, location, totalAttack) {
        ...
        buttons: {
            Dodge: {
                ...
                callback: async html => {
                    ...
                    rollFormula = addModifiersToFormula(skill.modifiers, rollFormula)
