import {ApplyDamage, BlockAttack, ExecuteDefence} from "./actions.js";
...
function onDamage(event) {
    ...
    if (touchedLocation.name !== "randomSpell") {
        ...
        touchedLocation = actor.getRandomLocationObjectForRandomEnemy(witcher.locations.randomHuman.name);
...
export async function rollDamage(img, name, damageFormula, location, locationFormula, strike, effects, damageType) {
    ...
    switch (damageType) {
        case "slashing":
            damageTypeloc = "WITCHER.Armor.Slashing";
            break;
        case "bludgeoning":
            damageTypeloc = "WITCHER.Armor.Bludgeoning";
            break;
        case "piercing":
            damageTypeloc = "WITCHER.Armor.Piercing";
            break;
        case "elemental":
            damageTypeloc = "WITCHER.Armor.Elemental";
            break;
    }
    ...
    await (await new Roll(damageFormula).evaluate({async: true})).toMessage(messageData)
}
